...
Its flagship feature is the **Sequential Batch Backtester**: Instead of doing dozens of tests manually – manually selecting an EA, clicking start, waiting for the test to finish, changing the symbol, and clicking start again – you can define an entire sequence of combinations at once. The platform automatically iterates through your EAs, currency pairs, and timeframes back-to-back while you can focus on other things. **This saves an enormous amount of manual work and time.** 

It dynamically generates beautiful, entirely offline HTML summary reports with perfectly anti-aliased Java2D equity charts for all combined runs.

---

## ✨ Core Features

### 1. 🔁 Sequential Multi-Backtesting (Eliminate Manual Work)
The days of clicking "Start" for every strategy permutation are over.
...
- **Fault Tolerance**: If a specific EA or chart configuration fails to load, the engine seamlessly logs the error and proceeds to the next run without halting your entire overnight batch.

![Multi-Backtester UI](images/multi-backtester-config.png)

### 2. 📊 High-Performance Offline Reporting
The platform totally divorces itself from MT5's notoriously flawed internal image and HTML generator.
- **Native Data Parsing**: Dynamically parses the raw historical result data, securely extracting metrics and the full tick-by-tick equity history.
...
- **Aggregated HTML Summary**: Compiles the results of an entire batch run into a single, compact, and beautiful `multi_report.html` file. Native Base64 embedded charts ensure you can examine every single test run in one neat document, and even share it effortlessly via email.
...
- **Graphics**: `java.awt.Graphics2D` hardware-accelerated anti-aliasing, linear gradients, and responsive text sizing for offline Equity generation.
