...
* **Modularer Task-Ketten-Editor**:
  ...
  - **Getrenntes Diversitäts-Clustering**: Das Zahnrad eines Clustering-Tasks öffnet einen eigenen Konfigurationsdialog für Modulname, Quell-/Ziel-Databank, Parameter- und Trade-Differenz, Mindestzahl unterschiedlicher Parameter sowie die maximale Zielanzahl. Der `(i)`-Button im Dialog erklärt Algorithmus, Schwellenwerte und Auswahlbeispiele ausführlich. Der Task arbeitet ausschließlich auf der gewählten Quelle und besitzt keine gekoppelten Kurzzeit-/Langzeit-Performancefilter. Sollen sowohl Kurzzeit- als auch Langzeit-Ergebnisse geclustert werden, werden zwei Clustering-Tasks angelegt. Der zweite liest die Ausgabedatabank des Langzeit-Retesters.
